#Main File for Primary key creation, resampling, column extracting and appending 
#of all 77 files for each specific frequecy
import pandas as pd
import glob
import os
import io
...
while i<len(symbol):
    try:
        ...
        df = pd.read_csv(io.StringIO(corpus[i]), index_col=0, sep=',', names=['Open','High', 'Low','Close','Volume'], parse_dates=True)
