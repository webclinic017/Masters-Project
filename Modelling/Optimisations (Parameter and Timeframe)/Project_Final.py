#Time frame and parameter Optimisation - 54,000 seconds / 15 hour completion 
import numpy as np
import pandas as pd
import math
import pandas as pd
...
STRATEGY_PARAMS = dict(dat = 1, period = 17, devfactor= 1.0)
...
toolbox.register("mutate", tools.mutUniformInt, low=[0,10,1], up=[7,151,4], indpb=0.2)
# selection strategy
...
toolbox.register("attr_dat", random.randint, 0, 7)
toolbox.register("attr_period", random.randint, 10, 151)
toolbox.register("attr_devfactor", random.randint, 1, 4)
toolbox.register(
    "individual",
    tools.initCycle,
    creator.Individual,
